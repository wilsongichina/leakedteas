...
const sourceRoot = path.join(root, "Set 1");
const localSetDir = path.join(root, "sets", "set-1");
const publicSetDir = path.join(root, "public", "sets", "set-1");
...
const subjectSources = {
  reading: {
    ...
    file: "ATI TEAS Version 7 - Reading Set 1 - ChatGPT Extracted Questions.html",
    duration: "00:55:00"
  },
  math: {
    label: "Math",
    dir: "ATI TEAS Version 7 - Math",
    file: "ATI TEAS Version 7 - Math Set 1 - ChatGPT Extracted Questions.html",
    duration: "00:57:00"
  },
  science: {
    label: "Science",
    dir: "ATI TEAS Version 7 - Science",
    file: "ATI TEAS Version 7 - Science Set 1 - ChatGPT Extracted Questions.html",
    duration: "01:00:00"
  },
  english: {
    label: "English",
    dir: "ATI TEAS Version 7 - English Language and Usage",
    file: "ATI TEAS Version 7 - English Set 1 - ChatGPT Extracted Questions.html",
    duration: "00:37:00"
  }
};
...
const questionImages = {
  reading: {
    42: { source: "q42-phone.system.png", output: "reading-q42.png" }
  },
  math: {
    6: { source: "q6-math-800-students.png", output: "math-q6.png" }
  },
  science: {
    25: { source: "q25-growth-curve.png", output: "science-q25.png" }
  }
};
...
function hasInlineImage(question) {
  return /<img/i.test(String(question.stimulusHtml || "")) || /<img/i.test(String(question.question || ""));
}
...
function buildQuizData() {
  ...
  for (const [key, source] of Object.entries(subjectSources)) {
    ...
    const questions = extractQuestions(filePath).map((question, index) => ({
      ...
      image: !hasInlineImage(question) && questionImages[key]?.[Number(question.number || index + 1)]
        ? `assets/${questionImages[key][Number(question.number || index + 1)].output}`
        : "",
      imageAlt: !hasInlineImage(question) && questionImages[key]?.[Number(question.number || index + 1)]
        ? `${source.label} question ${Number(question.number || index + 1)} diagram`
        : "",
      options: Array.isArray(question.options) ? question.options : [],
      correct: getCorrectIndex(question),
      explanation: "Answer extracted from the uploaded Set 1 question screenshot."
    }));
...
function copySubjectAssets() {
  ...
  for (const source of Object.values(subjectSources)) {
    copyDirectoryAssets(path.join(sourceRoot, source.dir, "set1-assets"));
    copyDirectoryAssets(path.join(sourceRoot, source.dir, "extracted-diagrams"));
  }

  for (const [subjectKey, images] of Object.entries(questionImages)) {
    const sourceDir = path.join(sourceRoot, subjectSources[subjectKey].dir, "extracted-diagrams");
    for (const image of Object.values(images)) {
      const sourceFile = path.join(sourceDir, image.source);
      const targetLocal = path.join(localAssetsDir, image.output);
      const targetPublic = path.join(publicAssetsDir, image.output);
      if (!fs.existsSync(sourceFile)) continue;
      fs.copyFileSync(sourceFile, targetLocal);
      fs.copyFileSync(sourceFile, targetPublic);
    }
  }
...
function updateSetPage(templateFile, outputFile, quizData) {
  ...
  html = html.replace(/<title>.*?<\/title>/, "<title>TEAS Version 7 Set 1 Full Questions | Teas Gurus</title>");
  html = html.replace(
    /<meta name="description" content=".*?" \/>/,
    '<meta name="description" content="TEAS Version 7 Set 1 full question list with Reading, Math, Science, and English sections." />'
  );
  html = html.replace(/TEAS Version 7 Set 9/g, "TEAS Version 7 Set 1");
  html = html.replace(/Set 9 subject tabs/g, "Set 1 subject tabs");
  html = html.replace(/const quizData = [\s\S]*?\n\s*const quizPageConfig = /, `const quizData = ${JSON.stringify(quizData)};\n\n    const quizPageConfig = `);
  html = html.replace(/titlePrefix: "TEAS Version 7 Set 9"/, 'titlePrefix: "TEAS Version 7 Set 1"');
