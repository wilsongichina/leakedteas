...
const sourceRoot = path.join(root, "set 8");
const localSetDir = path.join(root, "sets", "set-8");
const publicSetDir = path.join(root, "public", "sets", "set-8");
...
const subjectSources = {
  reading: {
    label: "Reading",
    dir: "Reading",
    file: "ATI TEAS Version 7 - Reading Set 8 - ChatGPT Extracted Questions.html",
    duration: "00:55:00"
  },
  math: {
    label: "Math",
    dir: "Math",
    file: "ATI TEAS Version 7 - Math Set 8 - ChatGPT Extracted Questions.html",
    duration: "00:57:00"
  },
  science: {
    label: "Science",
    dir: "Science",
    file: "ATI TEAS Version 7 - Science Set 8 - ChatGPT Extracted Questions.html",
    duration: "01:00:00"
  },
  english: {
    label: "English",
    dir: "English",
    file: "ATI TEAS Version 7 - English Set 8 - ChatGPT Extracted Questions.html",
    duration: "00:37:00"
  }
};
...
function buildQuizData() {
  ...
  for (const [key, source] of Object.entries(subjectSources)) {
    ...
    const questions = extractQuestions(filePath).map((question, index) => ({
      ...
      explanation: "Answer extracted from the uploaded Set 8 question screenshot."
    }));
...
function copySubjectAssets() {
  ...
  for (const source of Object.values(subjectSources)) {
    copyDirectoryAssets(path.join(sourceRoot, source.dir, "set8-assets"));
...
function updateSetPage(templateFile, outputFile, quizData) {
  ...
  html = html.replace(/<title>.*?<\/title>/, "<title>TEAS Version 7 Set 8 Full Questions | Teas Gurus</title>");
  html = html.replace(
    /<meta name="description" content=".*?" \/>/,
    '<meta name="description" content="TEAS Version 7 Set 8 full question list with Reading, Math, Science, and English sections." />'
  );
  html = html.replace(/TEAS Version 7 Set 9/g, "TEAS Version 7 Set 8");
  html = html.replace(/Set 9 subject tabs/g, "Set 8 subject tabs");
  html = html.replace(/const quizData = [\s\S]*?\n\s*const quizPageConfig = /, `const quizData = ${JSON.stringify(quizData)};\n\n    const quizPageConfig = `);
  html = html.replace(/titlePrefix: "TEAS Version 7 Set 9"/, 'titlePrefix: "TEAS Version 7 Set 8"');
