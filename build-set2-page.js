...
const sourceRoot = path.join(root, "Set 2");
const localSetDir = path.join(root, "sets", "set-2");
const publicSetDir = path.join(root, "public", "sets", "set-2");
...
const subjectSources = {
  reading: {
    ...
    file: "ATI TEAS Version 7 - Reading Set 2 - ChatGPT Extracted Questions.html",
    duration: "00:55:00"
  },
  math: {
    label: "Math",
    dir: "ATI TEAS Version 7 - Math",
    file: "ATI TEAS Version 7 - Math Set 2 - ChatGPT Extracted Questions.html",
    duration: "00:57:00"
  },
  science: {
    label: "Science",
    dir: "ATI TEAS Version 7 - Science",
    file: "ATI TEAS Version 7 - Science Set 2 - ChatGPT Extracted Questions.html",
    duration: "01:00:00"
  },
  english: {
    label: "English",
    dir: "ATI TEAS Version 7 - English Language and Usage",
    file: "ATI TEAS Version 7 - English Set 2 - ChatGPT Extracted Questions.html",
    duration: "00:37:00"
  }
};
  ...
  for (const [key, source] of Object.entries(subjectSources)) {
    ...
    const questions = extractQuestions(filePath).map((question, index) => {
      ...
      return {
        ...
        explanation: "Answer extracted from the uploaded Set 2 question screenshot."
      };
    });
...
function copySubjectAssets() {
  ...
  for (const source of Object.values(subjectSources)) {
    copyDirectoryAssets(path.join(sourceRoot, source.dir, "set2-assets"));
...
function updateSetPage(templateFile, outputFile, quizData) {
  ...
  html = html.replace(/<title>.*?<\/title>/, "<title>TEAS Version 7 Set 2 Full Questions | Teas Gurus</title>");
  html = html.replace(
    /<meta name="description" content=".*?" \/>/,
    '<meta name="description" content="TEAS Version 7 Set 2 full question list with Reading, Math, Science, and English sections." />'
  );
  html = html.replace(/TEAS Version 7 Set 9/g, "TEAS Version 7 Set 2");
  html = html.replace(/Set 9 subject tabs/g, "Set 2 subject tabs");
  html = html.replace(/const quizData = [\s\S]*?\n\s*const quizPageConfig = /, `const quizData = ${JSON.stringify(quizData)};\n\n    const quizPageConfig = `);
  html = html.replace(/titlePrefix: "TEAS Version 7 Set 9"/, 'titlePrefix: "TEAS Version 7 Set 2"');
