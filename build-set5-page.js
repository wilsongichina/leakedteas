...
const sourceRoot = path.join(root, "Set 5");
const localSetDir = path.join(root, "sets", "set-5");
const publicSetDir = path.join(root, "public", "sets", "set-5");
...
const subjectSources = {
  math: {
    ...
    file: "ATI TEAS Version 7 - Math Set 5 - ChatGPT Extracted Questions.html",
    duration: "00:57:00"
  },
  science: {
    label: "Science",
  ...
  english: {
    ...
    file: "ATI TEAS Version 7 - English Set 5 - ChatGPT Extracted Questions.html",
    duration: "00:37:00"
  }
};
...
function buildQuizData() {
  ...
  for (const [key, source] of Object.entries(subjectSources)) {
    ...
    const questions = extractQuestions(filePath).map((question, index) => ({
      ...
      explanation: "Answer extracted from the uploaded Set 5 question screenshot."
    }));
...
function copySubjectAssets() {
  ...
  for (const source of Object.values(subjectSources)) {
    copyDirectoryAssets(path.join(sourceRoot, source.dir, "set5-assets"));
...
function updateSetPage(templateFile, outputFile, quizData) {
  ...
  html = html.replace(/<title>.*?<\/title>/, "<title>TEAS Version 7 Set 5 Full Questions | Teas Gurus</title>");
  html = html.replace(
    /<meta name="description" content=".*?" \/>/,
    '<meta name="description" content="TEAS Version 7 Set 5 question list with Math, Science, and English sections." />'
  );
  html = html.replace(/TEAS Version 7 Set 9/g, "TEAS Version 7 Set 5");
  html = html.replace(/Set 9 subject tabs/g, "Set 5 subject tabs");
  html = html.replace(/const quizData = [\s\S]*?\n\s*const quizPageConfig = /, `const quizData = ${JSON.stringify(quizData)};\n\n    const quizPageConfig = `);
  html = html.replace(/titlePrefix: "TEAS Version 7 Set 9"/, 'titlePrefix: "TEAS Version 7 Set 5"');
  html = html.replace(/defaultSubject: "reading"/, 'defaultSubject: "math"');
